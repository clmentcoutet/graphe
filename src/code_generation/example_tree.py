...
from src.code_generation.syntax.expression import IdentifierExpression, Literal, BinaryOperation, UnaryOperation, \
    CallExpression, BaseTypeExpression
...
def tree_two_functions():
    return Module(classes=[
        ClassDefinition(
            ...
            attributes=[
                AttributeDefinition(name=IdentifierExpression("count"), type=BaseTypeExpression("INTEGER"), initializer=Literal(0)),
                AttributeDefinition(name=IdentifierExpression("name"), type=BaseTypeExpression("STRING"), initializer=Literal(2)),
                AttributeDefinition(name=IdentifierExpression("base"), type=BaseTypeExpression("INTEGER"))  # No initializer
            ],
            methods=[
                FunctionDefinition(
                                ...
                                ReturnStatement(
                                    expression=BinaryOperation(
                                        ...
                                        right=CallExpression(
                                            callee=IdentifierExpression("factorial", super_class=IdentifierExpression("self")),
                                            arguments=[
                                                BinaryOperation(
                                                    left=IdentifierExpression("n"),
